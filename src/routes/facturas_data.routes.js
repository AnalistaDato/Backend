...
router.get("/proyecciones", async (req, res) => {
  try {
    ...
    const [totalRows] = await pool.query("SELECT COUNT(*) AS count FROM facturas WHERE estado = 'proyectado'");
    const recordsTotal = totalRows[0].count;

    // Obtener el total de registros filtrados
    const [filteredRows] = await pool.query(`SELECT COUNT(*) AS count FROM facturas ${whereClause}`, queryParams.slice(0, -2));
